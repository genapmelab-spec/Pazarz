@php
    ...
    $pendingSellersCount = \App\Models\Seller::where('verification_status', 'pending')->count();
@endphp
...
<a href="{{ route('admin.sellers.index', ['status' => 'pending']) }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium {{ str_starts_with($route, 'admin.sellers') ? 'bg-gray-100 text-gray-900' : 'text-gray-600 hover:bg-gray-100' }}">
    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
    Sellers
    @if($pendingSellersCount > 0)
        <span class="ml-auto inline-flex items-center justify-center w-5 h-5 text-[10px] font-bold bg-yellow-500 text-white rounded-full">
            {{ $pendingSellersCount }}
        </span>
    @endif
</a>
