...
export function ProductCard({ product, onWishlistToggle, className }: ProductCardProps) {
  ...
  if (product.variants && product.variants.length > 0) {
    ...
    if (prices.length > 0) {
      ...
      if (min !== max) {
        return (
          <div className={cn('group rounded-[16px] overflow-hidden bg-white hover:shadow-elevation-2 transition-all duration-300', className)}>
            <Link to={`/products/${product.slug}`} className="block">
              <div className="relative aspect-[3/4] bg-surface overflow-hidden">
                {primaryImage ? (
                  <img src={primaryImage} alt={product.name} className="w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-110" loading="lazy" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-text-muted">
                    <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                  </div>
                )}
                <div className="absolute top-0 left-0 w-0 h-[2px] bg-accent group-hover:w-full transition-all duration-500 z-10" />
              </div>
            </Link>
            <div className="p-3">
              <Link to={`/products/${product.slug}`}>
                <h4 className="text-sm font-medium text-text-primary line-clamp-2 mb-1 group-hover:text-accent transition-colors duration-200">{product.name}</h4>
              </Link>
    ...
    <div className={cn('group rounded-[16px] overflow-hidden bg-white hover:shadow-elevation-2 transition-all duration-300', className)}>
      <Link to={`/products/${product.slug}`} className="block">
        <div className="relative aspect-[3/4] bg-surface overflow-hidden">
          ...
          {/* Premium corner accent line on hover */}
          <div className="absolute top-0 left-0 w-0 h-[2px] bg-accent group-hover:w-full transition-all duration-500 z-10" />
          {onWishlistToggle && (
            <button onClick={(e) => { e.preventDefault(); onWishlistToggle(product.id) }} className="absolute top-3 right-3 w-8 h-8 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center hover:bg-white/40 transition-all duration-200 shadow-elevation-1 opacity-0 group-hover:opacity-100 z-10" aria-label={product.is_wishlisted ? 'Remove from wishlist' : 'Add to wishlist'}>
              <Heart className={cn('w-4 h-4 transition-colors', product.is_wishlisted ? 'fill-error text-error' : 'text-white')} />
            </button>
          )}
        </div>
      </Link>
